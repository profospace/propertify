...
app.get('/api/update_gallery_list', async (req, res) => {
  try {
    // Fetch all properties from the MongoDB collection
    const allProperties = await Property.find();

    // Loop through each property and update its galleryList key
    for (const property of allProperties) {
      // Rename the key from galleryList to gallery_list
      property.gallery_list = property.galleryList;
      delete property.galleryList;
    ...
    // Respond with a success message
    res.status(200).json({ message: 'Gallery list updated for all properties' });
  } catch (error) {
    console.error('Error updating gallery list for all properties:', error);
    res.status(500).json({ message: 'Internal Server Error' });
  }
});
