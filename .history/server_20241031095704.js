...
router.get('/builders/:id/buildings', async (req, res) => {
  try {
...
router.get('/builders/:id/properties', async (req, res) => {
  try {
...
router.post('/builders/:id/buildings', async (req, res) => {
  try {
...
router.post('/builders/:id/properties', async (req, res) => {
  try {
      const builder = await Builder.findById(req.params.id);
      if (!builder) {
          return res.status(404).json({ message: 'Builder not found' });
...
router.get('/builders/:id/portfolio', async (req, res) => {
  try {
      const builder = await Builder.findById(req.params.id);
      if (!builder) {
          return res.status(404).json({ message: 'Builder not found' });
...
router.put('/buildings/:buildingId/builder', async (req, res) => {
  try {
...
router.get('/builders/:id', async (req, res) => {
  try {
      const builder = await Builder.findById(req.params.id)
          .populate({
              path: 'projects',
...
// Update builder
router.put('/builders/:id', async (req, res) => {
  try {
...
router.get('/builders/:id/projects', async (req, res) => {
  try {
      const { status, type } = req.query;
      const filter = { builder: req.params.id };
...
router.post('/builders/:id/projects', async (req, res) => {
  try {
      const builder = await Builder.findById(req.params.id);
...
router.get('/builders/search/location', async (req, res) => {
  try {
...
router.get('/builders/:id/analytics', async (req, res) => {
  try {
      const builder = await Builder.findById(req.params.id);
