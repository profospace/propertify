...
app.post('/api/projects', upload.fields([
  ...
  try {
    ...
    // Handle post_image if it exists
    if (req.files && req.files['post_image']) {
      const postImageFile = req.files['post_image'][0];
      const postImageParams = {
        Bucket: process.env.AWS_BUCKET_NAME,
        Key: `project_images/${uuid.v4()}_${postImageFile.originalname}`,
        Body: fs.createReadStream(postImageFile.path),
      };
      const postImageResult = await s3.upload(postImageParams).promise();
      uploadedImages.push(postImageResult.Location);
      projectData.post_image = postImageResult.Location;
    }

    // Handle floor_plan_image if it exists
    if (req.files && req.files['floor_plan_image']) {
      const floorPlanFile = req.files['floor_plan_image'][0];
      ...
      uploadedImages.push(floorPlanResult.Location);
      projectData.floor_plan_image = floorPlanResult.Location;
    }
    ...
    if (req.files && req.files['galleryList']) {
      for (const galleryFile of req.files['galleryList']) {
        const galleryParams = {
          Bucket: process.env.AWS_BUCKET_NAME,
          Key: `gallery_images/${uuid.v4()}_${galleryFile.originalname}`,
          Body: fs.createReadStream(galleryFile.path),
        };
        const galleryResult = await s3.upload(galleryParams).promise();
        uploadedImages.push(galleryResult.Location);
      }
      projectData.galleryList = uploadedImages.slice(uploadedImages.length - req.files['galleryList'].length);
    }
