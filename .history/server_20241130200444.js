...
const Project = require('./models/Project'); // Add this line with your other imports
...
app.delete('/api/projects/:projectId/properties/:propertyId', async (req, res) => {
  try {
    const { projectId, propertyId } = req.params;
    console.log('Disconnecting property:', propertyId, 'from project:', projectId);
    ...
    // Remove property from project's connected properties array
    ...
    if (propertyIndex > -1) {
      ...
      if (property.available) {
        project.statistics.availableProperties = Math.max(0, (project.statistics.availableProperties || 0) - 1);
      }
    }

    // Remove project reference from property
    property.projectId = undefined;
    ...
    console.log('Successfully disconnected property from project');
...
// Get all properties connected to a project
...
// Get properties connected to a project
app.get('/api/projects/:projectId/connected-properties', async (req, res) => {
  try {
    const { projectId } = req.params;
    const { 
      status, 
    ...
    const [properties, total] = await Promise.all([
      Property.find(filter)
        ...
        .skip(skip)
        .limit(Number(limit))
        .select('post_id post_title type_name price address status area bedrooms bathrooms post_image'),
    ...
    const summary = {
      ...
      availableProperties: await Property.countDocuments({ ...filter, available: true }),
      averagePrice: await Property.aggregate([
        { $match: filter },
        { $group: { _id: null, avg: { $avg: '$price' } } }
      ]).then(result => result[0]?.avg || 0),
      propertyTypes: await Property.distinct('type_name', filter)
    };
    ...
    res.status(500).json({
      success: false,
      message: 'Error fetching connected properties',
      error: error.message
    });
  }
});
    ...
    const projectsSection = {
      ...
      projects: nearbyProjects.map(project => {
        ...
        project.gallery?.forEach(galleryItem => {
          const category = galleryItem.category?.toLowerCase() || 'other';
          if (galleryItem.images && Array.isArray(galleryItem.images)) {
            if (category in galleryImages) {
              galleryImages[category].push(...galleryItem.images);
            } else {
              galleryImages.other.push(...galleryItem.images);
            }
          }
        });
        ...
        const primaryImage =
          project.gallery?.[0]?.images?.[0] || // First image from gallery
          project.masterPlan || // Fallback to master plan
          'https://example.com/default-project.jpg'; // Default image
        ...
        return {
          ...
          gallery: {
            ...
            totalCount: Object.values(galleryImages)
