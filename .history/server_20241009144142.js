...
app.get('/api/properties/filter', async (req, res) => {
  ...
  try {
    ...
    if (bedrooms) {
      ...
      console.log(`Added bedrooms filter: ${bedrooms}`);
    }
    if (bathrooms) {
      filter.bathrooms = Number(bathrooms);
      console.log(`Added bathrooms filter: ${bathrooms}`);
    }
    if (purpose) {
      filter.purpose = purpose;
      console.log(`Added purpose filter: ${purpose}`);
    }
    if (priceMin || priceMax) {
      filter.price = {};
      if (priceMin) {
        filter.price.$gte = Number(priceMin);
        console.log(`Added minimum price filter: ${priceMin}`);
      }
      if (priceMax) {
        filter.price.$lte = Number(priceMax);
        console.log(`Added maximum price filter: ${priceMax}`);
      }
    }
    // Geospatial query
    if (latitude && longitude) {
      const radiusInKm = radius ? parseFloat(radius) : 50; // Default to 50 km if radius is not provided
      ...
      console.log(`Added geospatial filter: lat ${latitude}, lon ${longitude}, radius ${radiusInMeters}m (${radiusInKm}km)`);
    }
    // Type name filter
    if (type_name) {
      filter.type_name = { $in: Array.isArray(type_name) ? type_name : [type_name] };
      console.log(`Added type_name filter: ${JSON.stringify(filter.type_name)}`);
    }
    ...
    if (sort) {
      ...
      console.log(`Sorting by price in ${order === -1 ? 'descending' : 'ascending'} order`);
    } else {
      // Default sorting if not specified
      sortOption.price = 1;
      console.log('Using default sorting: price in ascending order');
    }
